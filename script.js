...
function Parallax(operator, imageUrl, minHeight, minWidth, animation, division) {
    ...
    if (imageUrl != "") {
        ...
        console.error("Parallax: Vous ne pouvez pas crГ©er un parallax sans image." + "Si cette erreur persiste, il se pourrait que lвЂ™emplacement de votre image ne sois pas trouvГ©e en raison dвЂ™un chemin dвЂ™accГЁs erronГ©." + "Ou que lвЂ™attribut {@data-src} soit vide ou non inscrit dans votre fichier HTML");
    }
...
for (var i = 0; i < elements.length; i++) {
    if (elements[i].getAttribute("data-type") == "parallax") {
        ...
        if (elements[i].getAttribute(dataObjFit) != null) {
            ...
            // attributes.dataObjFit = "width: 100%;"
        }
